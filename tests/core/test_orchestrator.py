...
def test_process_prompt_success(mock_storage, mock_fb, mock_wp, mock_sheets, mock_ai, mock_telegram, mock_config):
    ...
    orch.fb.comment_on_post.assert_called_once_with(
        ...
        "Đọc chi tiết tại link sau: https://wp.url/story"
    )
...
def test_process_prompt_skips_image_when_disabled(mock_storage, mock_fb, mock_wp, mock_sheets, mock_ai, mock_telegram, mock_config):
    orch = Orchestrator(config=mock_config, disabled_steps=["ai_image_generation"])
...
def test_process_prompt_generates_image_when_enabled(mock_storage, mock_fb, mock_wp, mock_sheets, mock_ai, mock_telegram, mock_config):
    orch = Orchestrator(config=mock_config, disabled_steps=[])
...
def test_debug_saves_image_response_on_success(mock_storage, mock_fb, mock_wp, mock_sheets, mock_ai, mock_telegram, mock_config):
    orch = Orchestrator(config=mock_config, debug=True, disabled_steps=[])
...
def test_process_article_data_starts_at_publishing_steps(mock_storage, mock_fb, mock_wp, mock_sheets, mock_ai, mock_telegram, mock_config):
    ...
    assert result["url"] == "https://wp.url/article"
    assert result["outputs"]["wordpress_url"] == "https://wp.url/article"
    assert result["outputs"]["facebook_comment_state"] == "success"
    orch.ai.generate_story.assert_not_called()
    orch.ai.generate_image.assert_not_called()
    orch.wp.publish.assert_called_once_with(
        "Crawled Title",
        "Crawled content with enough words for publishing",
    ...
    orch.fb.comment_on_post.assert_called_once_with(
        orch.fb.publish_photo_caption.return_value,
        "Đọc chi tiết tại link sau: https://wp.url/article",
    )
    mock_telegram.assert_called_once()
...
def test_process_article_data_debug_saves_crawled_article_response(mock_storage, mock_fb, mock_wp, mock_sheets, mock_ai, mock_telegram, mock_config):
    orch = Orchestrator(config=mock_config, debug=True)
    orch.wp.publish.return_value = "https://wp.url/article"
    article = {
        "title": "Crawled Title",
