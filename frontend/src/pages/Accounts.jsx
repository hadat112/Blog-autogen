    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage your credentials for various services.</p>
        </div>
        <button
          onClick={() => { setEditingAccount(null); setShowForm(true); }}
          className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center space-x-2 hover:bg-blue-700 transition shadow-sm w-fit"
        >
          <Plus size={18} />
      ...
      {/* Tabs */}
      <div className="flex items-center space-x-1 border-b border-gray-200 overflow-x-auto no-scrollbar scroll-smooth shrink-0">
        {types.map(type => (
          <button
            key={type.id}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
        {filteredAccounts.length === 0 ? (
          <div className="col-span-full py-20 text-center bg-white rounded-xl border border-dashed border-gray-300">
            <p className="text-gray-500">No {activeType !== 'all' ? activeType.toUpperCase() : ''} accounts configured yet.</p>
          </div>
        ) : (
            ...
            <div key={acc.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
              <div className="flex justify-between items-start mb-4">
                <div className="overflow-hidden">
                  <h4 className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors truncate">{acc.name}</h4>
              ...
              <div className="text-xs text-gray-500 space-y-1 mb-4 font-mono overflow-hidden">
                {acc.type === 'wp' && <p className="truncate">URL: {acc.config.url}</p>}
                {acc.type === 'ai' && <p className="truncate">Model: {acc.config.text_model}</p>}
                {acc.type === 'fb' && <p className="truncate">Page: {acc.config.page_id}</p>}
                {acc.type === 'gs' && <p className="truncate">Sheet ID: {acc.config.spreadsheet_id}</p>}
                {acc.type === 'tg' && <p className="truncate">Chat ID: {acc.config.chat_id}</p>}
              </div>
